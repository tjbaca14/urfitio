...
class ContextRetriever(Protocol):
    ...
    Examples: CoachService, DocumentService, EmailContextService, etc.
    """

    def retrieve(self, query: Optional[dict]) -> Optional[str]:
        """
...
class RAGPipeline:
    ...
    async def generate(
        ...
        Flow:
        1. Retrieve context from domain provider
        ...
        Args:
            ...
            context_query: Query for context retrieval (domain-specific structure)
    ...
    def _retrieve_context(self, context_query: Optional[dict]) -> Optional[str]:
        """Retrieve context from domain provider."""
        if not context_query:
            return None

        context = self.context_retriever.retrieve(context_query)

        if context:
            logger.info("Context retrieved for RAG pipeline")
        else:
            logger.warning(f"No context found for query: {context_query}")
